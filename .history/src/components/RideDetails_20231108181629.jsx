...
// import sampleVideo from '../assets/images/sample_video.mp4'
...
const RideDetails = () => {
  ...
  return (
    <div className=' bg-white scroll-smooth -mt-5 md:top-0 top-3/4 absolute z-20 rounded-2xl p-2 shadow-lg flex flex-col md:m-2 h-max justify-center items-center w-full md:w-96'>
        ...
        <div className=' video-viewer m-1 p-1 w-full flex flex-col items-center justify-between'>
            ...
            <div className='video-viewer-1 h-60 z-20 relative w-full bg-black rounded-md flex gap-2 justify-center items-center'>
                { videoIsON ? 
                <video src='http://commondatastorage.googleapis.com/gtv-videos-bucket/sample/SubaruOutbackOnStreetAndDirt.mp4' controls={false} autoPlay></video> : 
                <button onClick={() => controlVideo()} className=' pl-5 z-30 pr-5 pt-2 pb-2 bg-white hover:bg-pr-blue border-spacing-1 border text-black border-collapse hover:border-pr-blue hover:text-white rounded-full border-white'>Show Video</button> }
            </div>
        </div>

        {/* Driver Detailed Info  */}
        
        <div className=' driver-details m-1 p-1 w-full flex flex-col items-center justify-between'>
            <h2 className=' text-lg text-pr-violet mb-3 font-semibold'>Driver Details</h2>
            <div className='driver-detaisl-1 w-fit flex flex-col gap-2 justify-center items-center'>
                ...
                <div className=' driver-name font-bold text-lg'>Pankaj Kumar</div>
            </div>
        </div>
    </div>
  )
